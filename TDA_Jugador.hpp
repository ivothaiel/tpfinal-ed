...
void cambio(tjugador &a, tjugador &b){
	    tjugador aux = a; // Variable auxiliar simple
	    a = b;
	    b = aux;
}
				...
				            aux = eliminar_jugador(arbol->izq, alias_buscado); 
					        } 
						        else if (comparacion > 0) { 
							...
							            aux = arbol;
							            if (arbol->izq == NULL && arbol->der == NULL) {  // Caso 1: Nodo Hoja
								                arbol = NULL; 
									            }
							            else if (arbol->izq == NULL) {  // Caso 2a: 1 Hijo Derecho
								                arbol = arbol->der;
							            } 
												             else if (arbol->der == NULL) {  // Caso 2b: 1 Hijo Izquierdo
													                 arbol = arbol->izq;
												             }
												             else { // Caso 3: 2 Hijos
