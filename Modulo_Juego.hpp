...
void seleccionarPalabrasAleatorias(tdiccionario dic, tpila &pila_juego, int totalPalabras) {
	// 1. Array est�tico de �ndices para las 6 palabras a seleccionar.
	indicesSeleccionados indices; 
	int indice_aleatorio;
	int indices_encontrados = 0;
	
	// Generar 6 �ndices aleatorios �nicos
	srand(time(NULL)); 
	...
	for (int i = 0; i < MAX_CLAVES; i++) {
		...
		while (p != NULL) {
			// Buscamos si el �ndice global actual (indice_global) coincide con uno de los 6 �ndices aleatorios
			for (int j = 0; j < 6; j++) {
